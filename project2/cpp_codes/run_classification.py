...
def grid_search():
    ...
    for i in range(x_len):
        for j in range(y_len):
            ...
            outfilename = "_".join(["accuracy", str(lamb), str(gamma)]) + ".txt"
    ...
    outfilename_val = "grid_search_lamb_gamma_val.txt"
    with open(outfilename_val, "w") as outfile:
        ...
        for i in range(x_len):
            for j in range(y_len):
                ...
                args = [str(lamb), str(gamma), str(accuracy_val[i,j])]
                line = " ".join(args)
                outfile.write(line)
                outfile.write("\n")

    outfilename_test = "grid_search_lamb_gamma_test.txt"
    with open(outfilename_test, "w") as outfile:
        outfile.write("lamb gamma accuracy_test\n")
        for i in range(x_len):
            for j in range(y_len):
                #nodes = num_nodes[i]
                #epochs = num_epochs[j]
